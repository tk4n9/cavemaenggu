...
const { safeWriteFlag, readFlag, VALID_MODES } = require('../hooks/cavemaenggu-config');
...
test('writes flag in normal (non-symlinked) directory', (tmp) => {
  ...
  const flagPath = path.join(flagDir, '.cavemaenggu-active');
...
test('writes flag when parent directory is a symlink owned by current user', (tmp) => {
  ...
  const flagPath = path.join(symlinkDir, '.cavemaenggu-active');
  safeWriteFlag(flagPath, 'ultra');

  // Flag should exist in the real directory
  const realFlagPath = path.join(realDir, '.cavemaenggu-active');
...
test('readFlag works through symlinked parent directory', (tmp) => {
  ...
  const result = readFlag(path.join(symlinkDir, '.cavemaenggu-active'));
...
test('safeWriteFlag then readFlag round-trip through symlink', (tmp) => {
  ...
  const flagPath = path.join(symlinkDir, '.cavemaenggu-active');
  safeWriteFlag(flagPath, 'maeng-gu-ultra');

  // Read back through the same symlink path
  const result = readFlag(flagPath);
  assert.strictEqual(result, 'maeng-gu-ultra');
});
...
test('refuses flag file that is itself a symlink (even through symlinked parent)', (tmp) => {
  ...
  safeWriteFlag(path.join(symlinkDir, '.cavemaenggu-active'), 'full');
...
test('readFlag refuses flag file that is a symlink', (tmp) => {
  ...
  fs.symlinkSync(secretFile, path.join(realDir, '.cavemaenggu-active'));

  const result = readFlag(path.join(realDir, '.cavemaenggu-active'));
...
test('symlink to nonexistent target silently fails', (tmp) => {
  ...
  assert.strictEqual(fs.existsSync(path.join(symlinkDir, '.cavemaenggu-active')), false);
});
...
test('safeWriteFlag no longer has blanket symlink parent refusal', (tmp) => {
  ...
  const source = fs.readFileSync(
    path.join(__dirname, '..', 'hooks', 'cavemaenggu-config.js'), 'utf8'
  );
