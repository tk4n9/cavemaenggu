#!/usr/bin/env node
// Tests for tools/cavemaenggu-init.js — fixture-based.
// Run: node tests/test_cavemaenggu_init.js
...
const INIT = path.join(ROOT, 'tools', 'cavemaenggu-init.js');

let passed = 0;
let failed = 0;

function test(name, fn) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cavemaenggu-init-test-'));
  try {
    fn(tmp);
    passed++;
    console.log(`  ✓ ${name}`);
...
console.log('cavemaenggu-init tests\n');

test('greenfield: creates all rule files with proper frontmatter', (tmp) => {
  execFileSync(process.execPath, [INIT, tmp], { encoding: 'utf8' });
  const cursor = fs.readFileSync(path.join(tmp, '.cursor/rules/cavemaenggu.mdc'), 'utf8');
  assert.match(cursor, /alwaysApply: true/);
  assert.match(cursor, /Respond terse like smart caveman/);
  const windsurf = fs.readFileSync(path.join(tmp, '.windsurf/rules/cavemaenggu.md'), 'utf8');
  assert.match(windsurf, /trigger: always_on/);
  const cline = fs.readFileSync(path.join(tmp, '.clinerules/cavemaenggu.md'), 'utf8');
...
test('skip mode: existing .cursor rule is not overwritten without --force', (tmp) => {
  ...
  fs.writeFileSync(path.join(dir, 'cavemaenggu.mdc'), '# original\nDo not delete me.\n');
  const out = execFileSync(process.execPath, [INIT, tmp], { encoding: 'utf8' });
  assert.match(out, /\? .*\.cursor\/rules\/cavemaenggu\.mdc/);
  const after = fs.readFileSync(path.join(dir, 'cavemaenggu.mdc'), 'utf8');
...
test('--only filters to one target', (tmp) => {
  ...
  assert.ok(fs.existsSync(path.join(tmp, '.clinerules/cavemaenggu.md')));
  assert.ok(!fs.existsSync(path.join(tmp, '.cursor')));
});
...
test('detects sentinel and skips files that already have caveman content', (tmp) => {
  ...
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, 'cavemaenggu.md'),
