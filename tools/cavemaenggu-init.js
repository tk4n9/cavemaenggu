#!/usr/bin/env node
// cavemaenggu init — drop the always-on cavemaenggu activation rule into a target
// repo for every IDE agent we support. Idempotent. Safe to re-run.
//
// Usage:
//   node tools/cavemaenggu-init.js [target-dir] [--dry-run] [--force] [--only <agent>]
//   curl -fsSL https://raw.githubusercontent.com/tk4n9/cavemaenggu/main/tools/cavemaenggu-init.js | node - [args]
//
...
// existing memory files — that's the job of `/cavemaenggu:compress`.

const fs = require('fs');
const path = require('path');

// Embedded so the tool works standalone (npx-style) without the rules/ dir.
// Mirrors rules/cavemaenggu-activate.md verbatim — keep these in sync.
...
Switch level: /cavemaenggu lite|full|ultra|maeng-gu (or /mg)
Stop: "stop caveman", "normal mode", "보통 모드", or "맹구 꺼"
...
const AGENTS = [
  { id: 'cursor',   file: '.cursor/rules/cavemaenggu.mdc',
    frontmatter: '---\ndescription: "Cavemaenggu mode — terse communication (caveman + 맹구 fork), ~75% fewer tokens, full technical accuracy"\nalwaysApply: true\n---\n\n',
    mode: 'replace' },
  { id: 'windsurf', file: '.windsurf/rules/cavemaenggu.md',
    frontmatter: '---\ntrigger: always_on\n---\n\n',
    mode: 'replace' },
  { id: 'cline',    file: '.clinerules/cavemaenggu.md',
    frontmatter: '',
    mode: 'replace' },
...
function loadRuleBody() {
  ...
  try {
    const local = path.join(__dirname, '..', 'rules', 'cavemaenggu-activate.md');
...
function help() {
  console.log(`cavemaenggu init — drop always-on cavemaenggu rule into a target repo

Usage: cavemaenggu-init.js [target-dir] [--dry-run] [--force] [--only <agent>]
...
function main() {
  ...
  console.log(`cavemaenggu init — ${opts.target}${opts.dryRun ? ' (dry run)' : ''}\n`);
