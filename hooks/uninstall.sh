#!/bin/bash
# cavemaenggu — uninstaller for the SessionStart + UserPromptSubmit hooks
# Removes: hook files in ~/.claude/hooks, settings.json entries, and the flag file
# Usage: bash hooks/uninstall.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/tk4n9/cavemaenggu/main/hooks/uninstall.sh)
set -e
...
if [ -d "$CLAUDE_DIR/plugins" ]; then
  if find "$CLAUDE_DIR/plugins" -path "*/cavemaenggu*" -name "plugin.json" -print -quit 2>/dev/null | grep -q .; then
    PLUGIN_INSTALLED=1
  fi
fi

if [ "$PLUGIN_INSTALLED" -eq 1 ]; then
  echo "Cavemaenggu appears to be installed as a Claude Code plugin."
  echo "To uninstall the plugin, run:"
  echo ""
  echo "  claude plugin disable cavemaenggu"
  echo ""
...
echo "Uninstalling cavemaenggu hooks..."
...
if [ -f "$SETTINGS" ]; then
  ...
  if ! command -v node >/dev/null 2>&1; then
    ...
    echo "         Remove the cavemaenggu SessionStart and UserPromptSubmit"
    ...
    CAVEMAENGGU_SETTINGS="$SETTINGS" CAVEMAENGGU_HOOKS_DIR="$HOOKS_DIR" node -e "
      ...
      // Remove statusLine if it references cavemaenggu
...
echo ""
echo "Other agents:"
echo "  npx skills remove cavemaenggu       # Cursor, Windsurf, Cline, Copilot, etc."
echo "  claude plugin disable cavemaenggu   # Claude Code plugin"
echo "  gemini extensions uninstall cavemaenggu  # Gemini CLI"
