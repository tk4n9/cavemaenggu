#!/usr/bin/env node
// cavemaenggu-stats — read the active Claude Code session log, print real token
// usage plus an estimated savings figure from the benchmark in benchmarks/.
//
// Run directly:    node hooks/cavemaenggu-stats.js
// Inside Claude:   /cavemaenggu-stats triggers this via the UserPromptSubmit hook.
...
const os = require('os');
const { readFlag, appendFlag, readHistory, safeWriteFlag } = require('./cavemaenggu-config');
...
// maeng-gu modes show no estimate until benchmarked. Add an entry here when a
// new run is committed.
...
// Detect *.original.md / *.md pairs left behind by cavemaenggu-compress. The
...
function formatHistory({ sessions, outputTokens, estSavedTokens, estSavedUsd, since }) {
  ...
  if (sessions === 0) {
    return `\nCavemaenggu Stats — Lifetime${window}\n${sep}\nNo sessions logged yet — run /cavemaenggu-stats inside any session to start tracking.\n${sep}\n`;
  }
...
function formatShare({ outputTokens, turns, mode, model }) {
  if (turns === 0) {
    return '🪨 caveman armed but no turns yet — github.com/tk4n9/cavemaenggu';
  }
  const ratio = COMPRESSION[mode] != null ? COMPRESSION[mode] : null;
  const price = priceForModel(model);
  ...
  if (ratio !== null) {
    ...
    return `🪨 Saved ${estSaved.toLocaleString()} output tokens${usd} across ${turns} turns this session — github.com/tk4n9/cavemaenggu`;
  }
  return `🪨 ${turns} turns, ${outputTokens.toLocaleString()} output tokens this session — github.com/tk4n9/cavemaenggu`;
}
...
function formatStats({ outputTokens, cacheReadTokens, turns, mode, model, sessionPath, compressed }) {
  ...
  if (ratio !== null) {
    ...
    savings = `Est. without cavemaenggu: ${estNormal.toLocaleString()}\n` +
    ...
    savings = 'Cavemaenggu not active this session.';
  }
...
function main() {
  ...
  const historyPath = path.join(claudeDir, '.cavemaenggu-history.jsonl');
  ...
  if (all || sinceArg) {
    ...
    if (sinceArg && sinceMs === null) {
      process.stderr.write(`cavemaenggu-stats: --since takes Nh or Nd (e.g. 7d, 24h), got: ${sinceArg}\n`);
      process.exit(2);
    }
  ...
  if (!sessionFile) {
    process.stderr.write('cavemaenggu-stats: no Claude Code session found.\n');
    process.exit(1);
  }

  const parsed = parseSession(sessionFile);
  const mode = readFlag(path.join(claudeDir, '.cavemaenggu-active'));

  // Append a snapshot of this session's totals to the lifetime log. Multiple
  // /cavemaenggu-stats calls in one session emit multiple lines for the same
  ...
  if (parsed.turns > 0) {
    ...
    // cat without parsing JSONL. Updated on every /cavemaenggu-stats run.
    // Routed through safeWriteFlag — the suffix path is predictable and
    // user-owned, same symlink-clobber surface as the .cavemaenggu-active flag.
    const agg = aggregateHistory(historyPath, null);
    const suffix = agg.estSavedTokens > 0 ? `⛏ ${humanizeTokens(agg.estSavedTokens)}` : '';
    safeWriteFlag(path.join(claudeDir, '.cavemaenggu-statusline-suffix'), suffix);
  }
