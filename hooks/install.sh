#!/bin/bash
# cavemaenggu — one-command hook installer for Claude Code
# Installs: SessionStart hook (auto-load rules) + UserPromptSubmit hook (mode tracking)
# Usage: bash hooks/install.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/tk4n9/cavemaenggu/main/hooks/install.sh)
...
if ! command -v node >/dev/null 2>&1; then
  echo "ERROR: 'node' is required to install the cavemaenggu hooks (used to merge"
...
if [ "$FORCE" -eq 0 ]; then
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ -f "$SETTINGS" ]; then
    if CAVEMAENGGU_SETTINGS="$SETTINGS" node -e "
      const fs = require('fs');
      const settings = JSON.parse(fs.readFileSync(process.env.CAVEMAENGGU_SETTINGS, 'utf8'));
      const hasCavemaengguHook = (event) =>
        Array.isArray(settings.hooks?.[event]) &&
        settings.hooks[event].some(e =>
          e.hooks && e.hooks.some(h => h.command && h.command.includes('cavemaenggu'))
        );
      process.exit(
        hasCavemaengguHook('SessionStart') &&
        hasCavemaengguHook('UserPromptSubmit') &&
        !!settings.statusLine
          ? 0
          : 1
      );
  ...
  if [ "$ALL_FILES_PRESENT" -eq 1 ] && [ "$HOOKS_WIRED" -eq 1 ] && [ "$HAS_STATUSLINE" -eq 1 ]; then
    ALREADY_INSTALLED=1
    echo "Cavemaenggu hooks already installed in $HOOKS_DIR"
...
if [ "$FORCE" -eq 1 ] && [ -f "$HOOKS_DIR/cavemaenggu-activate.js" ]; then
  echo "Reinstalling cavemaenggu hooks (--force)..."
else
  echo "Installing cavemaenggu hooks..."
fi
...
chmod +x "$HOOKS_DIR/cavemaenggu-statusline.sh"
...
CAVEMAENGGU_SETTINGS="$SETTINGS" CAVEMAENGGU_HOOKS_DIR="$HOOKS_DIR" node -e "
  const fs = require('fs');
  const settingsPath = process.env.CAVEMAENGGU_SETTINGS;
  const hooksDir = process.env.CAVEMAENGGU_HOOKS_DIR;
  const managedStatusLinePath = hooksDir + '/cavemaenggu-statusline.sh';
  const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
  if (!settings.hooks) settings.hooks = {};

  // SessionStart — auto-load cavemaenggu rules
  ...
  if (!hasStart) {
    settings.hooks.SessionStart.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/cavemaenggu-activate.js\"',
        timeout: 5,
        statusMessage: 'Loading cavemaenggu mode...'
      }]
    });
  }

  // UserPromptSubmit — track mode changes when user types /cavemaenggu commands
  ...
  if (!hasPrompt) {
    settings.hooks.UserPromptSubmit.push({
      hooks: [{
        type: 'command',
        command: 'node \"' + hooksDir + '/cavemaenggu-mode-tracker.js\"',
        timeout: 5,
        statusMessage: 'Tracking cavemaenggu mode...'
      }]
    });
  }

  // Statusline — wire cavemaenggu badge (report if skipped)
  if (!settings.statusLine) {
    ...
    if (cmd.includes(managedStatusLinePath)) {
      ...
      console.log('  NOTE: Existing statusline detected — cavemaenggu badge NOT added.');
...
echo "  - SessionStart hook: auto-loads cavemaenggu rules every session"
echo "  - Mode tracker hook: updates statusline badge when you switch modes"
echo "    (/cavemaenggu lite, /cavemaenggu ultra, /cavemaenggu-commit, /mg, etc.)"
echo "  - Statusline badge: shows [MAENGGU] or [MAENGGU:ULTRA] etc."
