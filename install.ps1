# cavemaenggu — smart multi-agent installer (Windows / PowerShell).
#
# One line:
#   irm https://raw.githubusercontent.com/tk4n9/cavemaenggu/main/install.ps1 | iex
#
...
$InitScriptUrl = "$RawBase/tools/cavemaenggu-init.js"
$McpShrinkPkg = "cavemaenggu-shrink"

# ── Help ────────────────────────────────────────────────────────────────────
if ($Help) {
@"
cavemaenggu installer (Windows) — detects your agents and installs cavemaenggu for each.
...
FLAGS
  ...
  -WithMcpShrink   Claude Code: register cavemaenggu-shrink MCP proxy. ON by default.
...
URLS THE INSTALLER MAY FETCH FROM
  ...
  $RawBase/tools/cavemaenggu-init.js
"@ | Write-Host
  exit 0
}
...
function Install-Claude {
  ...
  if (-not $Force) {
    try {
      ...
      if ($list -match "(?i)cavemaenggu") { $alreadyInstalled = $true }
    } catch {}
  }
  if ($alreadyInstalled) {
    ...
    if ((Try-Run "claude" @("plugin", "marketplace", "add", $Repo)) -and
        (Try-Run "claude" @("plugin", "install", "cavemaenggu@cavemaenggu"))) {
    ...
    if ($DryRun) {
      ...
      try {
        if ($localPs1) {
          ...
          $tmp = Join-Path $env:TEMP "cavemaenggu-hooks-install-$([Guid]::NewGuid()).ps1"
  ...
  # registering an `npx -y cavemaenggu-shrink` entry that 404s on every spawn.
  if ($WithMcpShrink) {
    Say "  → wiring cavemaenggu-shrink MCP proxy (-WithMcpShrink)"
    if (Has-Cmd "npm") {
      $packageOnNpm = $false
      ...
      if (-not $packageOnNpm) {
        ...
        Record-Skipped "cavemaenggu-shrink" "npm registry probe failed"
        Write-Host ""
        return
      }
    }
    ...
    if ($hasMcpAdd) {
      if ($DryRun) {
        Note "    would run: claude mcp add cavemaenggu-shrink -- npx -y $McpShrinkPkg"
      } else {
        if (Try-Run "claude" @("mcp", "add", "cavemaenggu-shrink", "--", "npx", "-y", $McpShrinkPkg)) {
          Record-Installed "cavemaenggu-shrink"
          ...
          Record-Failed "cavemaenggu-shrink" "claude mcp add failed"
        }
      }
    } else {
      ...
      Write-Host '    }'
      Write-Host ""
      Record-Skipped "cavemaenggu-shrink" "manual config required (snippet printed)"
    }
  }
  Write-Host ""
}
...
function Run-Init {
  ...
  if ($RepoRoot -and (Test-Path (Join-Path $RepoRoot "tools\cavemaenggu-init.js")) -and (Has-Cmd "node")) {
    if (Try-Run "node" (@((Join-Path $RepoRoot "tools\cavemaenggu-init.js")) + $initArgs)) { return $true }
    return $false
  }
  ...
  $tmp = Join-Path $env:TEMP "cavemaenggu-init-$([Guid]::NewGuid()).js"
  try {
...
if ($WithInit) {
  ...
  if (Run-Init) {
    Record-Installed "cavemaenggu-init ($PWD)"
  } else {
    Record-Failed "cavemaenggu-init" "tools/cavemaenggu-init.js failed"
  }
  Write-Host ""
